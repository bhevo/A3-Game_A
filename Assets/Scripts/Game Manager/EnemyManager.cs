...
public class EnemyManager : MonoBehaviour
{
    ...
    public Transform[] cannibal_SpawnPoints, boar_SpawnPointsl;

    [SerializeField]
    private int cannibal_Enemy_Count, boar_Enemy_Count;

    private int initial_Cannibal_Count, innitial_Boar_Count;

    public float wait_Before_Spawn_Enemies_Time = 1f;
   ...
   void Start()
    {
        ...
        innitial_Boar_Count = boar_Enemy_Count;
    ...
    void SpawnBoars()
    {
        ...
        {
            if (index >= boar_SpawnPointsl.Length)
            {
                index = 0;
            }
            Instantiate(boar_Prefab, boar_SpawnPointsl[index].position, Quaternion.identity);
            index++;
        }
    ...
    public void EnemyDied(bool cannibal)
    {
        ...
        {
            boar_Enemy_Count++;
            if(boar_Enemy_Count > innitial_Boar_Count)
            {
                boar_Enemy_Count = innitial_Boar_Count;
            }
        }
    }
